...
<template>
    <div class="flex justify-center gap-10">
        <div v-for="(poster, i) in posters" :key="`poster-${poster.id}` ?? `poster-${i}`">
            <div class="tw-wall-poster text-slate-900 flex flex-col gap-2" :style="{ backgroundColor: getRandomColor(), cursor: isEditMode ? 'initial' : 'pointer' }">
                <div class="bg-white mb-auto w-full flex justify-end py-2">
                    <div class="px-2 h-6 flex items-center">
                        <button v-show="!isEditMode" @click="enterEditMode(poster, i)">
                ...
                <div class="flex flex-col gap-4 p-2">
                    ...
                    <div v-if="isEditMode && (currentThematicId === poster.id)" class="flex items-center gap-2 flex-wrap">
                        ...
                        <div class="mt-6 mb-4 flex w-full justify-center items-center gap-4 px-2">
                            <button class="group bg-green-300 w-16 h-16 rounded-full shadow-lg flex items-center justify-center hover:bg-green-400" @click="editThematic(poster.id)">
                                <i class="fas fa-check text-4xl mx-5 group-hover:scale-x-110 transition-all"></i>
                            </button>
                            <button class="group bg-pink-300 w-16 h-16 rounded-full shadow-lg flex items-center justify-center hover:bg-pink-400" @click="exitEditMode()">
